...
if(isset($_GET['shop'])){
	...
	if(isset($_GET['customer']) && trim($_GET['customer'])!=""){
		...
		$where.=" and (`invoice_id` in (SELECT `id` from `invoice` where `customer_id`='$customer'))";
		
	}
	if(isset($_GET['query']) && trim($_GET['query'])!=""){
		$query=$_GET['query'];
		$where.=" and (`invoice_id`='$query' or model like '%$query%')";
	}
	//echo $where;
	$order="id";
	$limit="$offset,$count";
...
<table class="table-striped table-bordered table-hover table-condensed" id="c_list" style="width:100%" >
    ...
    <tr>
    	...
    	<td align="center">
        	<a href="print.php?inv_id=<?php echo $products['invoice_id']; ?>" class="btn btn-danger btn-sm"><i class="fa fa-print"></i></a>
		</td>
    </tr>
    <?php
			}	
	...
	?>
    <tr>
    	<td align="center" class="text-danger" colspan="12">No Records Found!!</td>
    </tr>
    <?php
		}
		if($pages>1){
	...
	?>	
    		<ul class="pagination pagination-sm">
    			<li><a onClick="navigate('<?php if(isset($_GET['customer'])){echo $_GET['customer'];} ?>','<?php if(isset($_GET['query'])){echo $_GET['query'];} ?>','<?php echo $page-1; ?>')">Prev</a></li>
          	</ul>
    <?php
			}
			for($i=1;$i<=$pages;$i++){
	...
	?>	
    		<ul class="pagination pagination-sm">
    			<li <?php if($i==$page){echo "class='active'";} ?>>
                	<a onClick="navigate('<?php if(isset($_GET['customer'])){echo $_GET['customer'];} ?>','<?php if(isset($_GET['query'])){echo $_GET['query'];} ?>','<?php echo $i; ?>')"><?php echo $i; ?></a>
                </li>
          	</ul>
    <?php		
				}
	...
	?>
    		<ul class="pagination pagination-sm">
    			<li><a onClick="navigate('<?php if(isset($_GET['customer'])){echo $_GET['customer'];} ?>','<?php if(isset($_GET['query'])){echo $_GET['query'];} ?>','<?php echo $page+1; ?>')">Next</a></li>
          	</ul>
    <?php
			}
	?>
