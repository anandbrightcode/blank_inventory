...
if(isset($_GET['customer'])){
	...
	$query="SELECT DATE_FORMAT(t1.date, '%d/%m/%Y') as `Date`, t2.`name` as `customer`, round(t1.`amount`,2) as `paid`
			from cust_pay_details t1, customer t2 where t1.`customer_id`=t2.`id` and (t1.date >= '$from' and t1.date <= '$to')";
		//echo $query;
	...
	$query="SELECT DATE_FORMAT(t1.date, '%d/%m/%Y') as `Date`, t2.`name` as `customer`, round(t1.`amount`,2) as `paid`
			from sup_pay_details t1, supplier t2 where t1.`supplier_id`=t2.`id` and (t1.date >= '$from' and t1.date <= '$to')";
