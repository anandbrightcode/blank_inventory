...
if(isset($_GET['shop'])){
	...
	$pagefilters="";
	$swhere="shop='$shop'";
	if(isset($_GET['supplier1']) && trim($_GET['supplier1'])!=""){
		$supplier=$_GET['supplier1'];
		$swhere.=" and `supplier_id`='$supplier'";
		$pagefilters.="&supplier=$supplier";
	}
	else{exit;}
	if(isset($_GET['sfrom1']) && isset($_GET['sto1'])){
		$from=$_GET['sfrom1'];$to=$_GET['sto1'];
		if($from!='' && $to!=''){
	...
	/*$arr=$obj->get_details("`purchase`","sum(`total_amount`) as `total`, sum(`paid`) as `paid`, sum(`dues`) as `dues`",$swhere);
