...
if(isset($_GET['shop'])){
	...
	elseif(isset($_GET['supplier']) && trim($_GET['supplier'])!=""){
		$supplier=$_GET['supplier'];
		$where="`supplier`='$supplier' and shop='$shop'";
	}
	else{
		$where="`shop`='$shop'";
	}
	...
	$gettotal=$obj->get_details("`purchase`","sum(`total_amount`) as `total`, sum(`paid`) as `paid`, sum(`dues`) as `dues`","`shop`='$shop'");
...
<table class="table-striped table-bordered table-hover table-condensed" id="c_list" style="width:100%" >
    ...
    <tr>
    	...
    	<td align="center">
			<?php if($purchase['supplier']!=0){?>
            <a href="supplierdetails.php?pagename=report&id=<?php echo $purchase['supplier']; ?>" class="btn btn-info btn-xs fa fa-eye" ></a>
            <?php } ?>
        </td>
    </tr>
    <?php
				$gross+=$purchase['gross_amount']; $trans+=$purchase['transport']; $total+=$purchase['total_amount'];
				$paid+=$purchase['paid']; $dues+=$purchase['dues'];
			}	
	...
	?>	
    		<ul class="pagination pagination-sm">
    			<li><a href="supplier.php?pagename=report&page=<?php echo $page-1; 
	...
	?>	
    		<ul class="pagination pagination-sm">
    			<li <?php if($i==$page){echo "class='active'";} ?>>
                	<a href="supplier.php?pagename=report&page=<?php echo $i; 
						if(isset($_GET['query'])){echo "&query=".$_GET['query'];}
						if(isset($_GET['supplier'])){echo "&supplier=".$_GET['supplier'];}
					?>"><?php echo $i; ?></a>
                </li>
