...
if(isset($_GET['shop'])){
	...
	$pagefilters="";
	$where="`shop`='$shop'";
	if(isset($_GET['customer1']) && trim($_GET['customer1'])!=""){
		$customer=$_GET['customer1'];
	...
	/*$arr=$obj->get_details("`invoice`","sum(`total_amount`) as `total`, sum(`paid`) as `paid`, sum(`dues`) as `dues`",$where);
