...
session_start();
  if(isset($_SESSION['user'])){
	...
	if(isset($_GET['id'])){
		$id=$_GET['id'];
		$supplier=$obj->get_details("`customer`","`name`","`id`='$id' and `shop`='$shop'");
	}
	else{
		header("Location:customer.php?pagename=report");	
	}
?>
    ...
    <?php include "../header.php";?>
   		<div class="container">
     		<div class="row">
                ...
                <div class="col-md-10">
                	<center><font size="+2">Customer Payment</font></center><hr />
					<?php
                     ...
                     <form action="../action/update.php" method="post" class="row" onsubmit="return Validate()">
                        <div class="col-md-12 table-responsive">
                            ...
                            <table class="table">
                                ...
                                <?php
									$arr=$obj->get_details("`invoice`","sum(`total_amount`) as `total`, sum(`paid`) as `paid`, sum(`dues`) as `dues`","`customer_id`='$id' and `shop`='$shop'");
									$array=$obj->get_rows("`invoice`","concat(`prefix`,`invoice_no`) as `invoice`, `date`, `total_amount`, `paid`, `dues`","`customer_id`='$id' and `dues`!=0 and `shop`='$shop'");
                                    if(is_array($array)){
                        ...
                        <div class="col-md-12 text-center">
                            ...
                            <input type="submit" name="update_cust_dues" value="Save" class="btn btn-success btn-sm" />
                        </div>
                    </form>
                </div>
            </div><!-- end of row -->
