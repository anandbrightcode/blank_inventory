...
session_start();
  ...
  $obj=new database();
	$category=array();
	$selcat=$obj->get_rows("`category`","*","`shop`='$shop' and `id` in (select `category` from stock)");
	if(is_array($selcat)){
...
<div class="container">
  <div class="row">
    ...
    <div class="col-md-10">
         <div class="row">
           <div class="col-md-12">
              <span class="text-center text-danger"><h2><i class="fa fa-file-text-o"></i> Quotation</h2></span>
              
           </div><!--end of column for invoice -->
         </div><!-- end of row for heading invoice -->
         <br />
        <form action="../action/insert.php" method="post" style="padding-top:5px;" class="row" id="idForm" onsubmit="return validatePrint()">
        	<div class="row">
                ...
                <b>Billing Mode</b>
                	<select name="billing_mode" id="billing_mode" onchange="reset_item(this.value);" class="form-control">
                    	<!--<option value="retail" selected="selected">Retail Quotation</option>-->
                	...
                	<table class="table table-condensed" style="margin-bottom:0;">
                    	...
                    	<tr>
                            ...
                            <td>
                            	...
                            	<input type="hidden" name="qbutton" id="qbutton" /><input type="hidden" name="shop" id="shop" value="<?php echo $shop; ?>" />
                            </td>
                            <td>
                            	<b>Discount</b>
                        ...
                        <tr>
                        	...
                        	<td align="center" style="vertical-align:middle; display: none;" >
                                <div class="radio">
                                    <label>
                	...
                	<table class="table table-condensed" style="margin-bottom:0;">
                    	<tr>
                            ...
                            <td width="20%">
								...
								<input type="hidden" name="qutoservicebutton" id="qutoservicebutton" /><input type="hidden" name="shop" id="shop" value="<?php echo $shop; ?>" />
            ...
            <div class="row" id="producttemp">
            	...
            	<div class="col-md-12 table-responsive" id="quot_temp" style="max-height:300px; background-color:#FFFFFF; padding:0;">
                	<?php include('quot_temp.php'); ?>
                </div><!-- end of table div -->
            </div><!-- end of form row 6 --><br />
			<div class="row" id="servicetemp"  style="display:none;">
            	<div class="text-center" id="response" style="display:none;"></div>
            	<div class="col-md-12 table-responsive" id="quotationservice_temp" style="max-height:300px; background-color:#FFFFFF; padding:0;">
                	<?php include('quotationservice_temp.php'); ?>
                </div><!-- end of table div -->
            	...
            	<div class="col-md-12">
                	<table class="table table-bordered">
                        ...
                        <tr style="display: none;">
                        	<th>Paid</th>
                            <td><input type="number" name="paid" id="paid" class="form-control" autocomplete="off" onkeyup="calcDues(this.value)" value="<?php echo $total; ?>" /></td>
                        	<th>Dues</th>
                            <td><input type="text" name="dues" id="dues" class="form-control" readonly="readonly" value="0" /></td>
                        	<th>Next Payment</th>
                    	...
                    	<tr>
                        	...
                        	<th style="display: none;">Advance</th>
                            <td style="display: none;"><input type="text" name="advance" id="advance" class="form-control" readonly="readonly"/></td>
                        	<th colspan="2" style="text-align:center; display: none">
            ...
            <br />
            <div class="row">
                <div class="col-md-2 lead">Additional Terms</div>
                <div class="col-sm-5">
                    <p class="text-info">Select to Add/Edit Additional Terms:</p>
                    <?php
                        $add_terms=$obj->get_rows("`add_terms`");
                        if(is_array($add_terms)){
                            foreach($add_terms as $terms){
                    ?>
                    <div class="checkbox">
                        <label><input type="checkbox" class="add_terms" value="<?php echo $terms['id']; ?>"><?php echo $terms['terms']; ?></label>
                    </div>
                    <?php
                            }
                        }
                        else{
                            echo "<a href='../masterkey/?pagename=master&section=add_terms'>Add Additional Terms</a>";
                        }
                    ?>

                </div>
                <div class="col-sm-5">
                    <textarea name="add_terms" id="add_terms" rows="7" class="form-control"></textarea>
                </div>
            </div><br>
			<div class="row">
                <div class="col-md-2 lead">Terms &amp; Conditions</div>
                <div class="col-md-5">
                    <textarea name="terms" id="terms" class="form-control" rows="8"></textarea>
                </div>
                <div class="col-md-5 text-danger">*Note - Use Enter Key to add new Terms &amp; Conditions.</div>
            </div>
            <div class="row" id="avs">
                <center><input type="submit" name="save_quotation" id="savebutton" value="Save &amp; Print" class="btn btn-success btn-sm" /></center>
            </div>
			<div class="row" id="avd" style="display:none;">
                <center><input type="submit" name="qutoservice_invoice" id="serbutton" value="Save &amp; Print" class="btn btn-success btn-sm" /></center>
            </div>
       	</form><!-- end of form -->
...
<script type="text/javascript" language="javascript"> 
	...
	function getCode(str){
		...
		else{ /*$("input[name=gstval]").prop("checked",false);*/ $('#code').val('');}
	}
	...
	$('body').bind('keyup', function(e) {
		...
		if(position>=1){
			if(e.keyCode==40 && position!=count+1){
				...
				$('.add_terms').change(function(){
					var term=$(this).parent().text();
					var add_terms=$('#add_terms').val();
					if($(this).is(":checked")){
						if(add_terms=='' || add_terms=='\n'){ 
							add_terms=term; 
						}
						else{ 
							if(add_terms.search(term)==-1){
								add_terms+="\n"+term; 
							}
						}
					}
						...
						if(add_terms.search(term)!=-1){
							if(add_terms.search("\n"+term)!=-1){
								add_terms=add_terms.replace("\n"+term,"");
							}
							else if(add_terms.search("\n"+term)==-1 && add_terms.search(term+"\n")!=-1){
								add_terms=add_terms.replace(term+"\n","");
							}
							else{
								add_terms=add_terms.replace(term,"");
							}
	...
	function resetAmount(){
		...
		$('#check_advance').attr("checked",false);
		$('#transport').val('');$('#paid').val($('#temp_total').val());$('#dues').val('0');
...
//$('#quantity').val('');
		...
		if(squantity.value==''){
			...
			$.ajax({
			   ...
			   success: function(data)
			   {
					...
					$.ajax({
						type:"GET",
						url:"quotationservice_temp.php",
						data:{shop:shop},
						success: function(data){
							$('#quotationservice_temp').html(data);
							resetFields();
							resetAmount();
						}
					});
 	...
 	function deleteTemp(id){
		...
		if(confirm("Are you sure you want to Delete this?")){
			$.ajax({
				...
				data: {
					id:id,shop:shop,delete_qtemp:'delete_qtemp'
				},
	...
	function  deleteTempqutoservice(id){
		var shop='<?php echo $shop; ?>';
		if(confirm("Are you sure you want to Delete this?")){
			$.ajax({
				type: 'GET',
				url: '../action/delete.php',
				data: {
					id:id,shop:shop,delete_qtemp_service:'delete_qtemp_service'
				},
				success: function (data) //on recieve of reply
				{
					//alert(data); // show response from the php script.
