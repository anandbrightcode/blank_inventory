...
//error_reporting(0);
	...
	$obj=new database();
	$words=new notowords;
	$quotation=$obj->get_details("`qutoservice`","*","`id`='$inv_id' and `shop`='$shop'");
	$getcustomer=$obj->get_details("`customer`","*","`id`='$quotation[customer_id]'");
	$qlist=$obj->get_rows("`qutoservicelist`","*","`service_id`='$inv_id' and `shop`='$shop'");
		//print_r($qlist); die;
	$getstate=$obj->get_details("`state`","*","`id`='$quotation[state]'");
	$shop_details=$obj->get_details("`shop`","*","`id`='$shop'");
	$checkigst=$obj->get_details("`qutoservice`","sum(`ivalue`) as `ivalue`","`inv_id`='$inv_id'");
?>
<!DOCTYPE html>
<html>
    <head>
    ...
    <body>
            ...
            <table align="center" width="95%" height="950" border="1" cellpadding="0" cellspacing="0" id="table" >
                ...
                <tr height="25">
                	...
                	<td align="center"><?php echo toDecimal($product['samount']); ?></td>
                </tr>
                <?php	
				         $gstvalue+=$product['ser_gstvalue'];
                ...
                <?php if($quotation['sroundoff']!=0){ ?>
                <tr height="25">
                	<td></td>
                    <td align="center">Round Off</td>
                    <td></td><td></td><td></td><td></td>
                    <td align="center"><?php echo twoDigits($quotation['sroundoff']); ?></td>
                </tr>
                <?php }?>
				<tr height="30">
                	<td colspan="6" style="padding:10px 20px; font-size:18px;">GST - <?php echo $ser_gst; ?>%</td>
                    <th align="center" style="padding:5px; font-size:18px;"><?php echo toDecimal($gstvalue); ?></th>
                </tr>
               
                
                <tr height="30">
                	<td colspan="6" style="padding:5px 20px; font-size:18px;"><?php echo "Rs. ".$words->to_words($quotation['stotal_amount'])." Only"; ?></td>
                    <th align="center" style="padding:5px; font-size:18px;"><?php echo toDecimal($quotation['stotal_amount']); ?></th>
                </tr>
