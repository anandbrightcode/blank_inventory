...
session_start();
	...
	$quot_list=$obj->get_rows("`quot_list`","*","`quot_id`='$quot_id' and `shop`='$shop'");
	$shop_details=$obj->get_details("`shop`","*","`id`='$shop'");
	$checkigst=$obj->get_details("`quot_list`","sum(`ivalue`) as `ivalue`,sum(`sgst`+`cgst`+`igst`) as `gst`","`quot_id`='$quot_id'");
    ...
    $hsns=$obj->get_rows("`quot_list`","hsn,round(sum(`taxable`),2) as `taxable`,round(sum(`svalue`+`cvalue`+`ivalue`),2) as `gst_value`,sum(`sgst`+`cgst`+`igst`) as `gst`,round(sum(`amount`),2) as `amount`",
                            "`quot_id`='$quot_id' group by hsn");
?>
...
<html xmlns="http://www.w3.org/1999/xhtml" moznomarginboxes mozdisallowselectionprint>
    ...
    <body>
        ...
        ?>
        <div class="quotation" style="width:1000px;">
            <center><font size="+1">Quotation</font></center>
            <table align="center" width="95%" height="400" border="1" cellpadding="2" cellspacing="0" class="info-table" >
                <tr>
                        ...
                        <table width="100%" height="100%" border="1" cellpadding="2" cellspacing="0">
                            <tr>
                                <td>
                                    ...
                                    Email : <?php echo $shop_details['email']; ?>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    Consignee<br />
                                    <font size="+1"><?php echo $quotation['customer_name']; ?><br /></font>
                                    <?php echo $quotation['add_to']; ?><br />
                                    Ph : <?php echo $quotation['customer_mobile']; ?><br />
                                    <?php if($quotation['gst']!=''){echo "GSTIN - ".$quotation['gst']; }?><br />
                                    State : <?php echo $quotation['state'].", Code : ".$quotation['code'] ?><br />
                                </td>
                            </tr>
                            <tr>
                                <td>
                    ...
                    <td width="25%">Quotation No<br /><?php echo $quotation['prefix'].$quotation['invoice_no']; ?></td>
                    <td>Dated<br /><?php echo date('d-m-Y',strtotime($quotation['date'])); ?></td>
                </tr>
                <tr height="50">
                    <td>Delivery Note</td>
            ...
            <table align="center" width="95%" height="650" border="1" cellpadding="0" cellspacing="0" class="quotation-table" >
            	<tr height="20">
                ...
                <tr height="20">
                    ...
                    <th align="center"><?php echo toDecimal($quotation['total_amount']); ?></th>
                </tr>
                <tr height="20">
             		<td colspan="8">Amount Chargeable (in words)<br />
                		<font size="+1">Indian Rupees. <?php echo $words->to_words($quotation['total_amount'])." Only";?></font> 
               		</td>
             	</tr>
            </table>
            ...
            <?php } ?>
                <tr>
                    <td colspan="7" style="border-bottom: 0;">Tax Amount (in words) : Indian Rupees. <?php echo $words->to_words($amount); ?> Only</td>
                </tr>
                <tr>
                    <td colspan="3" style="border: 0; border-left:1px solid; ">Company PAN : <?php if($shop_details['gst']!=''){echo "GSTIN - ".$shop_details['gst']; } ?></td>
            ...
            <center>This is Computer Generated Quotation</center>
        ...
        <script language="javascript">
        	function closeThis(str){
				...
				if(page=='report'){
					...
					window.location="../invoice/quotation.php?pagename=invoice";	
				}
			}
        </script>
    </body>
