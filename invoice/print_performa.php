...
session_start();
	...
	$obj=new database();
	$words=new notowords;
	$invoice=$obj->get_details("`performa_invoice`","*","`id`='$inv_id' and `shop`='$shop'");
	$performa_list=$obj->get_rows("`performa_list`","*","`invoice_id`='$inv_id' and `shop`='$shop'");
	$shop_details=$obj->get_details("`shop`","*","`id`='$shop'");
	$checkigst=$obj->get_details("`performa_list`","sum(`ivalue`) as `ivalue`,sum(`sgst`+`cgst`+`igst`) as `gst`","`invoice_id`='$inv_id'");
    ...
    $hsns=$obj->get_rows("`performa_list`","hsn,round(sum(`taxable`),2) as `taxable`,round(sum(`svalue`+`cvalue`+`ivalue`),2) as `gst_value`,sum(`sgst`+`cgst`+`igst`) as `gst`,round(sum(`amount`),2) as `amount`",
...
<html xmlns="http://www.w3.org/1999/xhtml" moznomarginboxes mozdisallowselectionprint>
    ...
    <body>
        ...
        <div class="invoice" style="width:1000px;">
            <center><font size="+1">Performa Invoice</font></center>
            <div style="font-style: italic; float: right; margin-top: -20px; margin-right: 35px;"><?php //echo $text; ?></div>
            ...
            <table align="center" width="95%" height="650" border="1" cellpadding="0" cellspacing="0" class="invoice-table" >
            	<tr height="20">
        ...
        <script language="javascript">
        	function closeThis(str){
				var page=str;
				if(page=='report'){
					window.location="../reports/performa.php?pagename=report";
				}	
				else{
					window.location="../invoice/performa.php?pagename=invoice";	
				}
			}
        </script>
    </body>
